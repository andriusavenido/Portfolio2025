...
import companion from "/images/companionlanding.png";
import eams from "/images/eams.png";
import gpucaddy from "/images/gpucaddy.png";
...
const Block: React.FC<{ details: ProjectDetails }> = ({ details }) => {
  ...
  const imgSRC = imgs[details.img as keyof typeof imgs];
  ...
  return (
    <div className="static flex lg:flex-row flex-col items-start p-4 md:p-8 bg-[var(--background)] w-full min-h-80 my-2 rounded-xl">
      <img
        ...
        src={imgSRC} // Use imgSRC to set the src
        alt={details.title} // Add alt text for accessibility
      />
      <div className="flex flex-col ml-4">
        ...
        <a
          href={details.link}
          className="bg-[var(--primary)] font-semibold py-2 px-6 rounded-lg mx-auto mt-4 hover:bg-[var(--accent-300)]"
        >
          View Project
        </a>
